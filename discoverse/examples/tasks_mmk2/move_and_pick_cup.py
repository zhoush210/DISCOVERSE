...
if __name__ == "__main__":
    ...
    # parser.add_argument("--dim17", action="store_true", help="genegrate 17 joint num mmk2 data") # this task need move
    ...
    # else:
    #     cfg.io_dim = 19
    cfg.io_dim = 19 # this task need move

    save_dir = os.path.join(DISCOVERSE_ROOT_DIR, "data/mmk2_pick_box_and_disk")
    ...
    while sim_node.running:
        ...
        try:
            if stm.trigger():
                if stm.state_idx == 0: # 降高度
                ...
                elif stm.state_idx == 1: # 准备动作
                    tmat_box = get_body_tmat(sim_node.mj_data, "box_disk")
                    target_posi1 = tmat_box[:3, 3] + np.array([-0.15, -0.15, 0.13]) # y,x,z
                    target_posi2 = tmat_box[:3, 3] + np.array([0.15, -0.15, 0.13]) # y,x,z
                ...
                elif stm.state_idx == 2: # 伸到盒子前
                    tmat_box = get_body_tmat(sim_node.mj_data, "box_disk")
                    target_posi1 = tmat_box[:3, 3] + np.array([-0.15, 0.0, 0.13]) # y,x,z
                    target_posi2 = tmat_box[:3, 3] + np.array([0.15, 0.0, 0.13]) # y,x,z
                    sim_node.lft_arm_target_pose[:] = sim_node.get_tmat_wrt_mmk2base(target_posi1)
                    sim_node.rgt_arm_target_pose[:] = sim_node.get_tmat_wrt_mmk2base(target_posi2)
                    sim_node.setArmEndTarget(sim_node.lft_arm_target_pose, sim_node.arm_action, "l", sim_node.sensor_lft_arm_qpos, Rotation.from_euler('zyx', [np.pi / 2, -0.0551 + np.pi, np.pi / 8]).as_matrix())
                    sim_node.setArmEndTarget(sim_node.rgt_arm_target_pose, sim_node.arm_action, "r", sim_node.sensor_rgt_arm_qpos, Rotation.from_euler('zyx', [-np.pi / 2, -0.0551 + np.pi, -np.pi / 8]).as_matrix())
                    sim_node.tctr_lft_gripper[:] = 0
                elif stm.state_idx == 3: # 抱住盒子
                    tmat_box = get_body_tmat(sim_node.mj_data, "box_disk")
                    target_posi1 = tmat_box[:3, 3] + np.array([-0.08, 0.0, 0.13]) # y,x,z
                    target_posi2 = tmat_box[:3, 3] + np.array([0.08, 0.0, 0.13]) # y,x,z
                    sim_node.lft_arm_target_pose[:] = sim_node.get_tmat_wrt_mmk2base(target_posi1)
                    sim_node.rgt_arm_target_pose[:] = sim_node.get_tmat_wrt_mmk2base(target_posi2)
                    sim_node.setArmEndTarget(sim_node.lft_arm_target_pose, sim_node.arm_action, "l", sim_node.sensor_lft_arm_qpos, Rotation.from_euler('zyx', [ np.pi / 2, -0.0551 + np.pi, np.pi / 8]).as_matrix())
                    sim_node.setArmEndTarget(sim_node.rgt_arm_target_pose, sim_node.arm_action, "r", sim_node.sensor_rgt_arm_qpos, Rotation.from_euler('zyx', [-np.pi / 2, -0.0551 + np.pi, -np.pi / 8]).as_matrix())
                    sim_node.tctr_lft_gripper[:] = 0
                elif stm.state_idx == 4: # 抬升一定高度
                    tmat_box = get_body_tmat(sim_node.mj_data, "box_disk")
                    sim_node.tctr_slide[0] = 1.22 - 1.08 * tmat_box[2, 3] - 0.05
                elif stm.state_idx == 5: # 向后取出盒子
                    tmat_box = get_body_tmat(sim_node.mj_data, "box_disk")
                    ...
                    sim_node.tctr_lft_gripper[:] = 0
                elif stm.state_idx == 6: # 后退
                    action[0] = -0.3 # back speed
                    sim_node.delay_cnt = int(2.0/sim_node.delta_t)
                elif stm.state_idx == 7: # 刹车停止
                    action[0] = 0.0
                    sim_node.delay_cnt = int(1.0/sim_node.delta_t)
                elif stm.state_idx == 8: # 上升
                    sim_node.tctr_slide[0] = 0.0
                elif stm.state_idx == 9: # 左转
                    action[1] = 0.1
                    sim_node.delay_cnt = int(7.5/sim_node.delta_t)
                elif stm.state_idx == 10: # 刹车停止
                    action[1] = 0.0
                    sim_node.delay_cnt = int(1.0/sim_node.delta_t)
                elif stm.state_idx == 11: # 前进
                    action[0] = 0.3 # forward speed
                    action[1] = 0.0
                    sim_node.delay_cnt = int(1.5/sim_node.delta_t)
                elif stm.state_idx == 12: # 刹车停止
                    action[0] = 0.0
                    action[1] = 0.0
                    sim_node.delay_cnt = int(1.0/sim_node.delta_t)
                # place box
                elif stm.state_idx == 13: # 降高度
                    sim_node.tctr_slide[0] = 0.45
                elif stm.state_idx == 14: # 松开盒子
                    tmat_box = get_body_tmat(sim_node.mj_data, "box_disk")
                    target_posi1 = tmat_box[:3, 3] + np.array([0.15, 0.0, 0.13]) # y,x,z
                    target_posi2 = tmat_box[:3, 3] + np.array([-0.15, 0.0, 0.13]) # y,x,z
                    sim_node.lft_arm_target_pose[:] = sim_node.get_tmat_wrt_mmk2base(target_posi1)
                    ...
                    sim_node.setArmEndTarget(sim_node.rgt_arm_target_pose, sim_node.arm_action, "r", sim_node.sensor_rgt_arm_qpos, Rotation.from_euler('zyx', [-np.pi / 2, -0.0551 + np.pi, -np.pi / 8]).as_matrix())
                    sim_node.tctr_lft_gripper[:] = 0
                elif stm.state_idx == 15: # 上升
                    sim_node.tctr_slide[0] = 0
                # pick disk
                elif stm.state_idx == 16: # 低头
                    sim_node.tctr_head[1] = 0.9 # 低头得0.9 不然限位
                elif stm.state_idx == 17: # 伸到盘子上1
                    tmat_pan = get_body_tmat(sim_node.mj_data, "disk_01")
                    target_posi = tmat_pan[:3, 3] + np.array([0.0, 0.0, 0.15]) # y,x,z
                ...
                elif stm.state_idx == 18: # 伸到盘子上2
                    tmat_pan = get_body_tmat(sim_node.mj_data, "disk_01")
                    target_posi = tmat_pan[:3, 3] + np.array([0.0, 0.01, 0.02]) # y,x,z
                    euler_angles = Rotation.from_matrix(tmat_pan[:3, :3]).as_euler('zyx', degrees=False)
                    sim_node.lft_arm_target_pose[:] = sim_node.get_tmat_wrt_mmk2base(target_posi)
                    sim_node.setArmEndTarget(sim_node.lft_arm_target_pose, sim_node.arm_action, "l", sim_node.sensor_lft_arm_qpos, Rotation.from_euler('zyx', [0, -0.0551, euler_angles[1]]).as_matrix())
                    sim_node.tctr_lft_gripper[:] = 1
                elif stm.state_idx == 19: # 抓住盘子
                    tmat_pan = get_body_tmat(sim_node.mj_data, "disk_01")
                    ...
                    sim_node.setArmEndTarget(sim_node.lft_arm_target_pose, sim_node.arm_action, "l", sim_node.sensor_lft_arm_qpos, Rotation.from_euler('zyx', [0, -0.0551, euler_angles[1]]).as_matrix())
                    sim_node.tctr_lft_gripper[:] = 0
                elif stm.state_idx == 24: # 松开盘子
                    sim_node.tctr_lft_gripper[:] = 1
                    sim_node.delay_cnt = int(0.5/sim_node.delta_t)
                elif stm.state_idx == 25: # 抬手
                    tmat_box = get_body_tmat(sim_node.mj_data, "box_disk")
                    target_posi = tmat_box[:3, 3] + np.array([0.25, 0.0, 0.3]) # y,x,z
                    euler_angles = Rotation.from_matrix(tmat_box[:3, :3]).as_euler('zyx', degrees=False)
                    sim_node.lft_arm_target_pose[:] = sim_node.get_tmat_wrt_mmk2base(target_posi)
