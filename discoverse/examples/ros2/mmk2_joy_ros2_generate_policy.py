...
class MMK2ROS2GP(MMK2ROS2JoyCtl):
    ...
    def save_state_to_json(self):
        ...
        select_arm = input("请选择要移动的机械臂或移动（a/l/r/x/y）：")  # x表示前进后退，y表示左右旋转
        if not select_arm in {"a", "l", "r", "x","y"}:
            print(("无效输入，保存失败。"))
            return
        ...
        state_data = {
            ...
            "base": {
                "position": [round(coord, 3) for coord in self.mj_data.qpos[:3]],  # base的位置 目前移动直接用全局坐标
                "rotation": [round(coord, 3) for coord in self.mj_data.qpos[3:7]],  # base的旋转
                "movement": select_arm if select_arm in {"x","y"}  else "stop"  # base的移动状态
            },
            "left_arm": {
                ...
                "movement": "move" if select_arm == "l" else "stop"
            },
            "right_arm": {
                ...
                "movement": "move" if select_arm == "r" else "stop"
            },
